...
class CholerLinear(BaseModel):
    """The linear based model described in Choler et al. 2010 (ie. M1)
    ...
    def _apply_model(self,
        ...
        # Plant available water. Denoted W_e in Choler et al. 
        W_plant = np.zeros_like(precip).astype('float32')
        ...
        for i in range(1,n_timesteps):
            ...
            if i - L - 1 < 0:
                W_plant[i] = np.max([0, W[i] - a3])
            else:
                W_plant[i] = np.max([0, W[i] - a3])
            
            # Eq. 2a Soil Water Content
            ...
            # Eq. 2b Vegetation growth base on prior veg & plant avaialbe water
            V[i+1] = V[i] + a2 * (W_plant[i-L] - W_plant[i-L-1])
            
            # Constrain veg to plausable range
            # Condition iv
...
class CholerNonLinear(BaseModel):
    """The non-linear based model described in Choler et al. 2010 (ie. M2)
